...
export async function GET() {
  ...
  try {
    ...
    const [companyRes, bibleRes, newsRes, contentRes, voiceRes] = await Promise.all([
      ...
      safeQuery(sql`SELECT id FROM messaging_bibles mb JOIN companies c ON mb.company_id = c.id WHERE c.user_id = ${user.id} AND (mb.status = 'complete' OR mb.full_document IS NOT NULL) LIMIT 1`),
